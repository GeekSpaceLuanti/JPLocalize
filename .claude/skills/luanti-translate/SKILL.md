...
### 3. luanti-translator サブエージェントに並列で翻訳依頼

各 `batch*.in.json` を Read し、その JSON 配列を **そのままプロンプトに貼り付けて** Agent ツールを起動。**1 メッセージ内に全バッチの Agent 呼び出しを並列**で含めること。

- `subagent_type`: `"luanti-translator"` (`.claude/agents/luanti-translator.md` 定義)
- `description`: `"Translate Luanti .tr batch N/M"`
- `prompt`: バッチ JSON 配列を文字列でそのまま渡す (例 `["Close","Command: @1 @2", ...]`)

翻訳ルール本体・モデル指定 (Haiku) は agent 定義に寄せたので、ここから再掲しない。親→子に渡すのはキー配列のみで、親メッセージのコンテキスト消費を最小化する。
...
`tmp/<domain>.done.missing.json` を Read し、その JSON 配列をそのまま `luanti-translator` サブエージェントに 1 回投げる (プロンプトは配列のみ。テンプレ不要)。
